    ...
    return function(modules) {
        ...
        "./node_modules/zalgo-promise/src/exceptions.js": function(module, exports, __webpack_require__) {
            ...
            exports.dispatchPossiblyUnhandledError = function(err) {
                ...
                for (var j = 0; j < (0, _global.getGlobal)().possiblyUnhandledPromiseHandlers.length; j++) (0, _global.getGlobal)().possiblyUnhandledPromiseHandlers[j](err);
            };
...
class Bearer {
    constructor(args) {
        ...
        this.hasAuthorized = (scenarioId) => new Promise((resolve, reject) => {
            postRobot$2
                .send(this.iframe, Events$1.HAS_AUTHORIZED, {
                scenarioId,
        ...
        this.revokeAuthorization = (scenarioId) => {
            postRobot$2
                .send(this.iframe, Events$1.REVOKE, {
                scenarioId,
                clientId: Bearer.config.clientId
            })
                .then(() => {
        ...
        this.askAuthorizations = ({ scenarioId, setupId, authRefId = '' }) => {
            if (this.isSessionInitialized) {
                const AUTHORIZED_URL = `${Bearer.config.integrationHost}v1/auth/${scenarioId}?setupId=${setupId}&authId=${authRefId}`;
    ...
    static set _instance(bearerInstance) {
    ...
    set bearerConfig(config$$1) {
        window[BEARER_WINDOW_KEY] = config$$1;
    }
    get maybeInitialized() {
        if (!this.isSessionInitialized) {
    ...
    initSession() {
        if (window !== undefined && !document.querySelector(`#${IFRAME_NAME}`)) {
            postRobot$2.on(Events$1.SESSION_INITIALIZED, event => {
...
Bearer.onRevoked = (scenarioId, callback) => {
    console.debug('[BEARER]', 'register onRevoked', scenarioId);
    return Bearer.emitter.addListener(Events$1.REVOKED, (data) => {
        if (data.data.scenarioId === scenarioId) {
    ...
    return function (params = {}, init = {}) {
        return new Promise((resolve, reject) => {
            Bearer.instance.maybeInitialized
                .then(() => {
                const sentParams = Object.assign({}, params, baseParams, { clientId: getClientId() });
...
function intentRequest({ intentName, scenarioId, setupId }) {
 ...
 * Constants
 */
const BearerContext = 'bearerContext';
const setupId = 'setupId';
/**
 * Intent
 */
// Usage
...
function Intent(intentName, type = IntentType.FetchData) {
    return function (target, key) {
        const getter = () => {
            return function (params = {}) {
                ...
                const query = Object.assign({}, baseQuery, queryParams);
...
class BearerMissingReferenceId extends Error {
    constructor(group = 'feature') {
        super();
        this.group = group;
        this.message = `Attribute ${this.group}ReferenceId is missing. Cannot fetch data without any reference`;
    }
}
class BearerMissingScenarioId extends Error {
    constructor() {
        super(...arguments);
        this.message = 'Scenario ID is missing. Please add @RootComponent decorator above your class definition';
    }
}

const StateManager = bearerState;
